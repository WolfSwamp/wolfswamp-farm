...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Bonus multiplier to calculate the _from to _to block reward.
    ...
    */
    function pendingSwampWolf(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpTotal > 0) {
            uint256 SWPerBlock = swampWolfPerBlock();
    ...
    function safePayReferralCommission(address _user, uint256 _pending) internal {
        if (address(swampWolfReferral) != address(0) && referralCommissionRate > 0) {
            ...
            uint256 commissionAmount0 = totalCommissionAmount.mul(5000).div(10000);// 50% of totalCommissionAmount
            uint256 commissionAmount1 = totalCommissionAmount.mul(3000).div(10000);// 30% of totalCommissionAmount
            uint256 commissionAmount2 = totalCommissionAmount.mul(2000).div(10000);// 20% of totalCommissionAmount
            // Check for balance just in case if rounding error causes pool to not have enough SWAMPWOLF.
            uint256 swampWolfBal = swampWolf.balanceOf(address(this));
            if (commissionAmount0 > 0) {
                if (referrer0 != address(0)) {
                    ...
                    if (commissionAmount0 > swampWolfBal) {
                        swampWolf.masterBurn(swampWolfBal);
                    }
                    else {
                        swampWolf.masterBurn(commissionAmount0);
                    }
                }
            }
            if (commissionAmount1 > 0) {
                if (referrer1 != address(0)){
                    ...
                    if (commissionAmount1 > swampWolfBal) {
                        ...
                        swampWolf.masterBurn(commissionAmount1);
                    }
                }
            }
            if (commissionAmount2 > 0) {
                if (referrer2 != address(0)) {
                    ...
                    if (commissionAmount2 > swampWolfBal) {
                        ...
                        swampWolf.masterBurn(commissionAmount2);
                    }
                }
            }
        }
     ...
     */
    function setReferralCommissionRateBp(uint16 _referralCommissionRate) public onlyAdmin{
     ...
     */
    function setFeeCommissionRatesBp(uint16 _feeRate, uint16 _devRate, uint16 _questRate, uint16 _marketingRate) public onlyAdmin {
    ...
    function farmingStart() public onlyAdmin{
        require(startTime > block.timestamp && startBlock > block.number , "Already started");
